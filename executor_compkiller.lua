-- CompKiller single-file executor script
-- Paste this entire file into your executor (e.g., Synapse, KRNL)

-- Compkiller - Mini UI (single-file)
-- Author: 4lpaca (source)
-- Wrapped for direct executor usage by exporting to getgenv()
...
-- export to global so executor usage can access it
pcall(function() getgenv().Compkiller = Compkiller end)

-- USAGE EXAMPLE (auto-run when pasted)
local CK = getgenv().Compkiller
if not CK then
    warn("Compkiller not initialized")
    return
end

local window = CK.new("My UI", "rbxassetid://120245531583106")

if window.AddToggle then
    window:AddToggle("Toggle Example", false, function(val) print("Toggle:", val) end)
end

if window.AddButton then
    window:AddButton("Example Button", function() print("Button pressed") end)
end

if window.AddToggle and window.AddButton then
    print("Compkiller UI created successfully")
end
